...
class PlasmaGCodeParser():
    """
    A Gcode parser to modify the code generated by Vectric for a Mach2/3
    controlled mill and implement a workflow for a floating head plasma cutter.
    ...
    def getInFileName(self):
        ...
        inFileName = eg.fileopenbox(msg=msg, title=self.title, default='*', filetypes=None)

        if inFileName and len(inFileName)>1:
            return inFileName
        else:
            msg = "No File found. Do you want to quit?"
            response = eg.ynbox(msg=msg, title=self.title, choices=('Yes', 'No'))
            if response:
                self.getInFileName()
            else:
                sys.exit(0)
    ...
    def getOutFileName(self):
        ...
        outFileName = eg.filesavebox(msg=msg, title=self.title, default=default, filetypes=['.nc','.gcode','.ngc'])
